...
namespace Finbourne.Configuration.Sdk.Extensions.Tests
{
    ...
    public class ApiConfigurationTest
    {
        ...
        [Test]
        public void ApiConfiguration_HasMissingConfig_Missing_ConfigurationUrl_Returns_True()
        {
            ApiConfiguration config = GetApiConfiguration();
            config.ConfigurationUrl = String.Empty;
            Assert.IsTrue(config.HasMissingConfig());
        }

        [Test]
        public void ApiConfiguration_MissingConfig_Missing_ConfigurationUrl_Returns_Correct_String()
        {
            ApiConfiguration config = GetApiConfiguration();
            config.ConfigurationUrl = String.Empty;
            List<string> missingList = config.GetMissingConfig();
            Assert.That(missingList.Count, Is.EqualTo(1), "GetMissingConfig list contains one item");
            Assert.That(missingList[0], Is.EqualTo(nameof(config.ConfigurationUrl)), "GetMissingConfig list string correct");
        }
        ...
        private ApiConfiguration GetApiConfiguration() {
            ...
            {
                ConfigurationUrl = "https://sub-domain.lusid.com/api",
